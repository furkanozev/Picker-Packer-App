...
import 'package:picker_packer/provider/packer_change_photo_service.dart';
import 'package:picker_packer/provider/packer_change_profile_service.dart';
import '../flutter_flow/flutter_flow_theme.dart';
import 'package:flutter/material.dart';

class PackerEditProfileWidget extends StatefulWidget {
  final UserLogin user;
  final Packer_Profile profile;
  PackerEditProfileWidget(
      {Key key, @required this.user, @required this.profile})
      : super(key: key);

  @override
  _PackerEditProfileWidgetState createState() =>
      _PackerEditProfileWidgetState(user, profile);
}

class _PackerEditProfileWidgetState extends State<PackerEditProfileWidget>
  ...
  PackerChangeProfileService packerProfileService =
      new PackerChangeProfileService();
  PackerChangePhotoService packerPhotoService = new PackerChangePhotoService();

  @override
  void initState() {
    super.initState();
    packerProfileService.setAccountId(user.id);
    packerPhotoService.setAccountId(user.id);
  }
  ...
  _getFromGallery() async {
    ...
    if (pickedFile != null) {
      ...
      packerPhotoService.setChanges(base64Image);
      var res = await packerPhotoService.request();
      if (res == true) {
  ...
  /// Get from Camera
  _removePhoto() async {
    packerPhotoService.setChanges(null);
    var res = await packerPhotoService.request();
    if (res == true) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        duration: Duration(seconds: 4),
  ...
  changeProfile() async {
    var res = await packerProfileService.request();
    if (res == true) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        duration: Duration(seconds: 4),
        content: Text(
      ...
      profile.name = packerProfileService.name;
      profile.email = packerProfileService.email;
      profile.age = packerProfileService.age;
      profile.phone = packerProfileService.phone;
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        duration: Duration(seconds: 4),
        content: Text(
                            ...
                            FlatButton(
                              ...
                              onPressed: () {
                                ...
                                if (EmailValidator.validate(email)) {
                                  packerProfileService.setChanges(
